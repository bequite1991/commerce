...
const getUserInfo = (code, callback, error) => {
  Taro.getUserInfo({
    ...
    success (res) {
      // 全局存储用户信息
      // store.commit('storeUpdateTaroUser', res.userInfo)
      // Taro.setStorageSync("_TY_userinfo", res.userInfo);
    ...
    fail () {
      ...
      // TarosetStorageSync("_TY_userinfo", '');
...
const postLogin = (code, iv, encryptedData, userinfo, callback) => {
  ...
  request('/config/commerce_wx_auth',{
    ...
    if (data.ok) {
      ...
      Taro._TY_opendid = _openId;//设置全局变量
      const u = data.data.data;
      Taro.setStorageSync("_TY_U", u);// 用户信息存储在缓存中
      // TODO 判断openId是否在数据库中，或者直接后台判断返回用户信息
      let sessionId = data.data.si;
      if(sessionId){
        //设置到全局变量中,后面每次请求都cookie都带上sessionId
        Taro.setStorageSync("_TY_S", sessionId);
      }
...
const initSession = (openId, u, userinfo, callback) => {
  ...
  if(u && u.telphone){
    ...
    TarosetStorageSync("_TY_CurrentInfo", {
